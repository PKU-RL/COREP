...
from graph import StateToGraph, GAT
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.stable_gat = GAT(node_feature_dim, gat_hidden_dim, state_embed_dim, num_heads)
        self.flexible_gat = GAT(node_feature_dim, gat_hidden_dim, state_embed_dim, num_heads)
    ...
    def forward(self, actions, states, rewards, hidden_state, return_prior, sample=True, detach_every=None, freeze=False):
        """
        ...
        graphs, edge_indices = self.graph_encoder(states, 0.8, 0.8, True)
        data_list = []
        for g, ei in zip(graphs, edge_indices):
            data_list.append(Data(x=g, edge_index=ei))
        batch_graph = Batch.from_data_list(data_list)
        if freeze:
            with torch.no_grad():
                stable_graphs = self.stable_gat(batch_graph).view(states.shape[0], -1, self.num_nodes, self.state_embed_dim)
        else:
            stable_graphs = self.stable_gat(batch_graph).view(states.shape[0], -1, self.num_nodes, self.state_embed_dim)
        flexible_graphs = self.flexible_gat(batch_graph).view(states.shape[0], -1, self.num_nodes, self.state_embed_dim)
        
        stable_mean = stable_graphs.mean(dim=-2)
        flexible_mean = flexible_graphs.mean(dim=-2)

        hs = stable_mean + flexible_mean
        ha = self.action_encoder(actions)
        hr = self.reward_encoder(rewards)
        try:
            h = torch.cat((ha, hs, hr), dim=2)
        except:
            h = torch.cat((ha.unsqueeze(0), hs, hr), dim=2)
        ...
        return latent_sample, latent_mean, latent_logvar, output, stable_graphs, flexible_graphs
